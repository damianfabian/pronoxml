  ...
  (
    ...
    return
      <json:object type="object">
        ...
        <json:fullName>{$user/name}</json:fullName>
...
declare function auth:weblogin($username as xs:string*, $password as xs:string*)
{
  ...
  (
    ...
    let $__    := xdmp:set-session-field("logged-in-user", $user/name/text())
    let $__    := xdmp:set-session-field("username", $user/username/text())
              
    return
      element logged-in-user
      {
        element message    { "Login successful" },
        element fullName   { fn:concat($user/name/text()) }, 
...
declare function auth:clearSession($username)
{
  ...
  return auth:deleteToken(xdmp:node-uri($session))
};
...
declare function auth:findSessionByUserForCleanup($username)
{
  ...
  let $uri := cts:uris("",("document"), $query )
