...
declare function usr:save($user)
{
    if($user) then
    (
            let $validation := usr:_save(
                                 element user-profile
                                 {
                                   element username { $user/username/text() },
                                   element name { $user/name/text() }, 
                                   element email  { $user/email/text() }, 
            ...
            if($validation) then
                ...
                fn:concat("User already exists: ", $user/username/text())
    )
    else
        fn:concat("Please fill up all the fields the user doesn't have information")
  ...
  let $cmd :=
        ...
        (
          'declare variable $uri external;
           ...
           let $validation := fn:doc-available($uri)
           return
            if(fn:not($validation)) then
            (
